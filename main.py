...
def add_details():
    ...
    result = 'First Name:{}, \nLast Name:{},\nEmail:{},\nAge:{},\nDate of Birth:{},\nPhone Number:{},\nAddress:{}'.format(firstname,lastname, email, age,date_of_birth,phone,address)
    tab1_display.insert(tk.END, result)
    messagebox.showinfo(title= "Registro GUI", message = "Submitted to DataBase")
    #print ('result:', result)
...
#Structure & Layout
window  = Tk()
window.title("GYM 4L - System Access. ")
...
tab_control.add(tab1, text = f'{"Home"}')
tab_control.add(tab2, text = f'{"View"}')
tab_control.add(tab3, text = f'{"Search"}')
tab_control.add(tab4, text = f'{"Export"}')
...
#Add header labels
label1 = Label(tab1, text = "Welcome! Register a new Client", padx=5, pady=5)
...
calendar = DateEntry(tab1, width=30, textvariable=datebirth_raw_entry,background='grey',foreground='white',borderwidth=4,year=2010)
...
button1 = Button(tab1, text="Add", width=12,bg="green",fg="white", command = add_details)
button1.grid(row=8, column=1,padx=5, pady=5)


button2 = Button(tab1, text="Clear", width=12,bg="blue",fg="white", command = clear_text)
...
button3 = Button(tab1, text="Clear Result", width=12,bg="darkgrey",fg="white", command = clear_display_view)
...
button_view = Button(tab2, text = "View All" , width= 12, bg = "green", fg = "white", command= view_all_users )
...
tree.heading("#5",text="Birth")
tree.heading("#6",text="Address")
tree.heading("#7",text="PhoneNumber")
tree.grid(row=10,column=0,columnspan=3,padx=5,pady=5)
#xscrollbar = Scrollbar(tree, orient=HORIZONTAL)
#xscrollbar.pack(side=BOTTOM, fill=X)

#sbar = ttk.Scrollbar(tab2,orient = "horizontal")
#sbar.config(command=tree.xview)
#tree.configure(xscrollcommand=sbar.set)
#bar.grid(row=12, column=6, sticky = "we")
...
button_clearsearch = Button(tab3,text="Clear Search", width=12,bg="blue",fg="white",command=clear_entered_search)
button_clearsearch.grid(row=2,column=1,padx=10,pady=10)

button_clearresult = Button(tab3,text="Clear Result", width=12,bg="blue",fg="white",command=clear_searching_view)
button_clearresult.grid(row=2,column=2,padx=10,pady=10)

button_search = Button(tab3,text="Search",width =12,bg="blue",fg="white",command=search_user_by_name)
...
button_delete = Button(tab2,text="Delete Client",width=12,bg='red',fg='#fff',command=delete_product)
button_delete.grid(row=14,column=0,padx=10,pady=10)
#About
label_about = Label(tab5, text = "GYM 4L V 0.1.0 \nMaximilianohrra@gmail.com", padx = 5, pady = 5)
